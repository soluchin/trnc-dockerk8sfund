...
docker container create --name postgres-db --network appnet -e POSTGRES_USER=dbadmin -e POSTGRES_PASSWORD=admin -e POSTGRES_DB=mydb -v ./dbpostgres/db_init.sql:/docker-entrypoint-initdb.d/init.sql postgres